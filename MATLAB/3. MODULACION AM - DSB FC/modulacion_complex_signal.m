...
% Parametros de la señal moduladora
...
subplot(3,1,1);
plot(t, moduladora, 'b');
title('Senal Moduladora (Dominio del Tiempo)');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0 0.05]); % Mostrar solo 50 ms para mejor visualizacion
grid on;

% Señal portadora
subplot(3,1,2);
plot(t, portadora, 'r');
title('Senal Portadora (Dominio del Tiempo)');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0 0.005]); % Mostrar solo 5 ms para mejor visualizacion
grid on;

% Señal modulada AM
subplot(3,1,3);
plot(t, modulada, 'm');
title('Senal Modulada AM (Dominio del Tiempo)');
xlabel('Tiempo (s)');
ylabel('Amplitud');
xlim([0 0.05]); % Mostrar solo 50 ms para mejor visualizacion
grid on;
...
title('Espectro de la Senal Moduladora');
...
title('Espectro de la Senal Portadora');
xlabel('Frecuencia (Hz)');
ylabel('Magnitud');
...
title(['Espectro de la Senal Modulada AM (Ancho de Banda = ', num2str(2*fm), ' Hz)']);
xlabel('Frecuencia (Hz)');
ylabel('Magnitud');
xlim([-1200 1200]); % Enfoque en las frecuencias relevantes
grid on;
...
legend('Espectro AM', 'Limites de banda');
